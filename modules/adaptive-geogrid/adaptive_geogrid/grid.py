...
from ._geodesic import GeodesicMetric
from ._graph import GraphPartitionState
from ._power import power_assign
from ._warp import SinusoidalWarp
from ._utils import as_coordinate_array, boundary_mask, repair_polygonal_geometry, transform_xy
from .hierarchy import aggregate_level


_BUNDLE_FORMAT = "adaptive-geogrid"
_BUNDLE_FORMAT_VERSION = 5
_SUPPORTED_BUNDLE_FORMAT_VERSIONS = {1, 2, 3, 4, 5}


class AdaptiveGeoGrid:
    """A fitted fine-level adaptive grid plus optional bottom-up hierarchy.

    The fine level may be generated by the original power/Laguerre backend or by
    another supported tessellation mode such as warped Laguerre or isotropic geodesic space.
    ...
    def __init__(
        ...
        *,
        ...
        tessellation_mode: str = "power",
        warp: SinusoidalWarp | None = None,
        geodesic_metric: GeodesicMetric | None = None,
        graph_state: GraphPartitionState | None = None,
    ):
        ...
        self.tessellation_mode = str(tessellation_mode)
        self._warp = warp
        self._geodesic_metric = geodesic_metric
        self._graph_state = graph_state

        if self.tessellation_mode == "warped" and self._warp is None:
            raise ValueError("warped tessellation mode requires warp state")
        if self.tessellation_mode != "warped" and self._warp is not None:
            raise ValueError("warp state is only valid for tessellation_mode='warped'")
        if self.tessellation_mode == "geodesic" and self._geodesic_metric is None:
            raise ValueError("geodesic tessellation mode requires geodesic metric state")
        if self.tessellation_mode != "geodesic" and self._geodesic_metric is not None:
            raise ValueError(
                "geodesic metric state is only valid for tessellation_mode='geodesic'"
            )
        if self.tessellation_mode == "graph" and self._graph_state is None:
            raise ValueError("graph tessellation mode requires graph partition state")
        if self.tessellation_mode != "graph" and self._graph_state is not None:
            raise ValueError("graph state is only valid for tessellation_mode='graph'")
    ...
    def classify(
        ...
        *,
        ...
        if np.any(inside):
            ...
            if self.tessellation_mode == "geodesic":
                fine[inside] = self._geodesic_metric.classify_projected(projected)
            elif self.tessellation_mode == "graph":
                fine[inside] = self._graph_state.classify_projected(projected)
            else:
                classifier_points = (
                    self._warp.forward(projected)
                    if self.tessellation_mode == "warped"
                    else projected
                )
    ...
    def _write_bundle_directory(self, path: Path) -> None:
        ...
        metadata = {
            ...
            "tessellation_mode": self.tessellation_mode,
            "warp": self._warp.to_dict() if self._warp is not None else None,
            "geodesic": (
                self._geodesic_metric.to_metadata()
                if self._geodesic_metric is not None
                else None
            ),
            "graph": self._graph_state.to_metadata() if self._graph_state is not None else None,
        }
        ...
        if self._geodesic_metric is not None:
            state.update(
                geodesic_valid_mask=self._geodesic_metric.valid_mask.astype(np.uint8),
                geodesic_cost=np.asarray(self._geodesic_metric.cost, dtype=np.float32),
                geodesic_labels=np.asarray(self._geodesic_metric.labels, dtype=np.int32),
            )
        if self._graph_state is not None:
            state.update(
                graph_generator_points=np.asarray(
                    self._graph_state.generator_points, dtype=np.float64
                ),
                graph_generator_labels=np.asarray(
                    self._graph_state.generator_labels, dtype=np.int32
                ),
            )
        np.savez_compressed(path / "state.npz", **state)
        ...
        # Keep the original projected geometry as part of the bundle. Reconstructing
        # global polygons by round-tripping them through WGS84 can introduce long
        # antimeridian seam edges, especially for coarse hierarchy levels.
        boundary_projected = gpd.GeoDataFrame(
            {"geometry": [self.boundary_projected]},
            crs=self.projected_crs,
        )
        boundary_projected.to_file(path / "boundary_projected.gpkg", driver="GPKG")

        for i, gdf in enumerate(self._levels_wgs84):
            gdf.to_file(path / f"level_{i}.geojson", driver="GeoJSON")
            self._levels_projected[i].to_file(
                path / f"level_{i}_projected.gpkg",
                driver="GPKG",
            )
    ...
    def to_directory(self, path: str | Path) -> None:
        ...
        fitted classifier state required for fast classification. Reload it
    ...
    def _load_directory(cls, path: Path) -> "AdaptiveGeoGrid":
        ...
        if format_version not in _SUPPORTED_BUNDLE_FORMAT_VERSIONS:
            raise ValueError(
                f"Unsupported grid bundle version {metadata.get('format_version')!r}; "
                f"supported versions are {sorted(_SUPPORTED_BUNDLE_FORMAT_VERSIONS)}"
            )
        ...
        boundary_wgs84 = repair_polygonal_geometry(boundary_gdf.geometry.union_all())

        projected_level_paths = [
            path / f"level_{i}_projected.gpkg" for i in range(n_levels)
        ]
        projected_boundary_path = path / "boundary_projected.gpkg"
        has_projected_geometry = (
            projected_boundary_path.exists()
            and all(level_path.exists() for level_path in projected_level_paths)
        )

        if format_version >= 5 and not has_projected_geometry:
            raise ValueError(
                "Serialized grid bundle v5 is missing projected geometry files"
            )

        if has_projected_geometry:
            projected_boundary_gdf = gpd.read_file(projected_boundary_path).to_crs(crs)
            boundary_projected = repair_polygonal_geometry(
                projected_boundary_gdf.geometry.union_all()
            )
            levels_projected = []
            for level_path in projected_level_paths:
                projected = gpd.read_file(level_path).to_crs(crs).reset_index(drop=True)
                projected["geometry"] = projected.geometry.map(repair_polygonal_geometry)
                levels_projected.append(projected)
        else:
            # Backward compatibility for v1-v4 bundles. This round-trip can be
            # lossy for global antimeridian-crossing polygons, so newly saved
            # bundles always include the projected geometry directly.
            boundary_projected = repair_polygonal_geometry(
                boundary_gdf.to_crs(crs).geometry.union_all()
            )
            levels_projected = []
            for gdf in levels_wgs84:
                projected = gdf.to_crs(crs).copy()
                projected["geometry"] = projected.geometry.map(repair_polygonal_geometry)
                levels_projected.append(projected)
        ...
        with np.load(state_path) as state:
            ...
            geodesic_valid_mask = (
                np.asarray(state["geodesic_valid_mask"], dtype=bool)
                if "geodesic_valid_mask" in state.files
                else None
            )
            geodesic_cost = (
                np.asarray(state["geodesic_cost"], dtype=float)
                if "geodesic_cost" in state.files
                else None
            )
            geodesic_labels = (
                np.asarray(state["geodesic_labels"], dtype=np.int32)
                if "geodesic_labels" in state.files
                else None
            )
            graph_generator_points = (
                np.asarray(state["graph_generator_points"], dtype=float)
                if "graph_generator_points" in state.files
                else None
            )
            graph_generator_labels = (
                np.asarray(state["graph_generator_labels"], dtype=np.int32)
                if "graph_generator_labels" in state.files
                else None
            )

        tessellation_mode = str(metadata.get("tessellation_mode", "power"))
        warp_data = metadata.get("warp")
        warp = SinusoidalWarp.from_dict(warp_data) if warp_data is not None else None

        geodesic_data = metadata.get("geodesic")
        geodesic_metric = None
        if geodesic_data is not None:
            if (
                geodesic_valid_mask is None
                or geodesic_cost is None
                or geodesic_labels is None
            ):
                raise ValueError("Serialized geodesic grid is missing raster state arrays")
            geodesic_metric = GeodesicMetric.from_metadata(
                geodesic_data,
                valid_mask=geodesic_valid_mask,
                cost=geodesic_cost,
        ...
        if graph_data is not None:
            if graph_generator_points is None or graph_generator_labels is None:
                raise ValueError("Serialized graph grid is missing generator state arrays")
            graph_state = GraphPartitionState.from_metadata(
                graph_data,
                generator_points=graph_generator_points,
                generator_labels=graph_generator_labels,
            )

        grid = cls(
            fine_tiles_projected=levels_projected[0],
        ...
        grid._levels_projected = levels_projected
        grid._parent_maps = []
    ...
    def _hierarchy_lane_bands_projected(
        ...
        *,
        ...
        for geom in self._levels_projected[level].geometry:
            ...
            outer = repair_polygonal_geometry(outer)
            if outer.is_empty:
                continue

            inner = repair_polygonal_geometry(geom.buffer(-(offset + width)))
            band = outer if inner.is_empty else outer.difference(inner)
            band = repair_polygonal_geometry(band)
    ...
    def plot(
        ...
        *,
        ...
        if points is not None:
            ...
            if len(sampled):
                sampled_projected = transform_xy(sampled, "EPSG:4326", self.projected_crs)
                ax.scatter(
                    sampled_projected[:, 0],
                    sampled_projected[:, 1],
                    s=point_size,
        ...
        if not base_line.is_empty:
            gpd.GeoSeries([base_line], crs=self.projected_crs).plot(
                ax=ax,
        ...
        if label_level is not None:
            label_gdf = self._levels_projected[label_level]
        ...
        if show_boundary:
            boundary_series = gpd.GeoSeries([self.boundary_projected], crs=self.projected_crs)
